...
- 관련 사례: [PostgreSQL Password Race Condition](../04-troubleshooting/secrets/README.md#1-postgresql-password-race-condition)
...
- 관련 사례: [Istio Gateway NodePort 불일치](../04-troubleshooting/infrastructure/README.md#2-istio-gateway-nodeport-불일치)
...
- 관련 사례: [Istio Gateway Sidecar Injection 실패](../04-troubleshooting/istio/README.md#5-istio-gateway-sidecar-injection-실패)

### GCP API 응답 타입 주의

GCP API가 반환하는 JSON의 필드 타입이 문서와 다를 수 있다. 특히 숫자 필드가 문자열로 반환되는 경우가 있으므로, 테스트 코드에서 타입을 유연하게 처리해야 한다.

- 관련 사례: [JSON 파싱 에러](../04-troubleshooting/testing/README.md#11-json-파싱-에러)
